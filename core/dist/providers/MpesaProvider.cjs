...
export class MpesaProvider {
  ...
  _validateCredentials() {
    ...
    if (missing.length > 0) {
      throw new ConfigurationError(`Missing mPesa credentials: ${missing.join(', ')}`, missing);
    }
  }
  async charge(request) {
    ...
    try {
      ...
      if (error.response) {
        throw new PaymentError(error.response.statusText || 'Payment failed', error.response.status, request.transactionReference);
      }
  ...
  async refund(transactionId, amount, reference) {
    ...
    if (!this.credentials.initiatorIdentifier || !this.credentials.securityCredential) {
      throw new ConfigurationError('Refund requires initiatorIdentifier and securityCredential');
    }
    try {
